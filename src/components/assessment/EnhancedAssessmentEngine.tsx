  ...
  const exportComprehensiveReport = () => {
    try {
      if (!comprehensiveResult) {
        console.error('Cannot generate comprehensive report: Result is null');
      ...
      if (!comprehensiveResult.executiveSummary || 
          typeof comprehensiveResult.overallScore === 'undefined' ||
          !comprehensiveResult.categoryScores ||
          !Array.isArray(comprehensiveResult.recommendations)) {
        console.error('Cannot generate report: Invalid comprehensive result structure', comprehensiveResult);
        alert('Assessment results are incomplete. Please retake the assessment.');
        return;
      }
...
EXECUTIVE SUMMARY
${comprehensiveResult.executiveSummary || 'No executive summary available.'}

OVERALL SCORE: ${comprehensiveResult.overallScore}%
Risk Level: ${(comprehensiveResult.riskLevel || 'unknown').toUpperCase()}

CATEGORY SCORES:
- HIPAA Compliance: ${comprehensiveResult.categoryScores?.hipaa || 0}%
- Ransomware Protection: ${comprehensiveResult.categoryScores?.ransomware || 0}%
- Dependency Management: ${comprehensiveResult.categoryScores?.dependency || 0}%
- Business Continuity: ${comprehensiveResult.categoryScores?.continuity || 0}%
...
${index + 1}. [${(rec.priority || 'medium').toUpperCase()}] ${rec.title || 'Untitled Recommendation'}
   Impact: Compliance +${rec.impact?.compliance || 0}%, Security +${rec.impact?.security || 0}%, Business +${rec.impact?.business || 0}%
   Cost: $${(rec.impact?.cost || 0).toLocaleString()}
   Timeline: ${rec.timeline || 'Not specified'}
   Effort: ${rec.effort || 'Not specified'}
   Description: ${rec.description || 'No description available'}
   Expected Improvement: ${rec.metrics?.before || 0}% → ${rec.metrics?.after || 0}% (+${rec.metrics?.improvement || 0}%)
   Resources Needed: ${rec.resources?.join(', ') || 'Not specified'}
`).join('\n')
...
Report ID: ${Date.now()}
    `;
      ...
      console.error('Error generating comprehensive assessment report:', error);
