...
const HealthEnhancementDashboard: React.FC<HealthEnhancementDashboardProps> = ({
  ...
  const [enhancementHistory, setEnhancementHistory] = useState<unknown[]>([]);
      ...
      {healthStatus?.issues && healthStatus.issues.length > 0 && (
        <Card className="p-6">
          ...
          <div className="space-y-3">
            {healthStatus.issues.map((issue: { id: string; message: string; severity: string; autoFixable?: boolean }, index: number) => (
