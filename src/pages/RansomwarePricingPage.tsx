...
const RansomwarePricingPage: React.FC = () => {
  ...
  const { startCheckout, isRedirecting } = useCheckout('ransomware');
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            {tiers.map((tier, idx) => {
              ...
              const canStartTrial = tierKey === 'professional' && isEligible('ransomware') && !hasActiveTrial;
                ...
                >
                  <Card className={`h-full flex flex-col border-2 ${tier.popular ? 'border-accent-500 dark:border-accent-600 shadow-lg' : 'border-gray-200 dark:border-gray-700'}`}>
                    ...
                    <div className="p-8 flex-grow">
                      ...
                      ) : tierKey === 'professional' && !isEligible('ransomware') ? (
                        <Button 
                          className="w-full" 
                          size="lg" 
                          variant={tier.popular ? 'primary' : 'outline'}
